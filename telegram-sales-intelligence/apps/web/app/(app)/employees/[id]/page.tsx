...
function Experience({ item }: { item: any }) {
  ...
  return (
    <article className="border-b border-line py-5 first:pt-0 last:border-0 last:pb-0">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {item.customerSegment && (
            <p className="mt-1 text-xs font-medium text-accent">{item.customerSegment}</p>
          )}
        </div>
        <div className="text-right">
          <p className="text-sm font-bold">{percent(item.confidenceScore)}</p>
          <p className="text-xs text-ink-subtle">{item.sampleSize} workflow</p>
        </div>
      </div>
      ...
      {segmentSignals && (
        <dl className="mt-4 grid gap-4 bg-canvas-subtle p-4 text-sm md:grid-cols-3">
          <div>
            <dt className="text-xs font-medium text-ink-subtle">Buying trigger</dt>
          ...
          <ol>
            {blueprint.map((stage, index) => (
              <li key={`${stage.stage}-${index}`} className="relative pb-4 pl-10 last:pb-0">
                ...
                <span
                  className="absolute left-0 top-0 grid h-8 w-8 place-items-center border border-accent/25 bg-accent-muted text-xs font-bold text-accent"
        ...
        steps.length > 0 && (
          <ol className="mt-4 grid gap-2 sm:grid-cols-2">
            {steps.map((step, index) => (
              <li
                key={`${step}-${index}`}
                className="flex gap-3 border border-line bg-canvas-subtle p-3 text-sm"
                style={{ borderRadius: 6 }}
              >
