    ...
    const artifacts = [
      {
        ...
        title: `Sales executive report ${dateLabel}`,
      },
      {
        id: crypto.randomUUID(),
        format: 'LATEX',
        ...
        title: `Sales executive report ${dateLabel} - LaTeX`,
      },
    ];
    return Promise.all(
...
function renderDailyReportHtml(date: Date, data: any): string {
  ...
  const attentionRows = (data.attentionConversations ?? [])
    ...
    .join('');
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Sales executive report</title><style>:root{--ink:#172033;--teal:#087f8c;--line:#dce2e9;--muted:#667085;--canvas:#f5f7fa}*{box-sizing:border-box}body{margin:0;background:var(--canvas);color:var(--ink);font-family:Arial,sans-serif}.cover{background:#172033;color:white;padding:56px max(6vw,40px)}.cover p{color:#c9d3df}.content{max-width:1180px;margin:0 auto;padding:36px}.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:12px}.kpi{background:white;border-top:3px solid var(--teal);padding:18px}.kpi strong{display:block;font-size:28px;margin-top:8px}.kpi span{font-size:12px;color:var(--muted);text-transform:uppercase}section{margin-top:32px;background:white;padding:24px}h1{font-size:34px;margin:0}h2{font-size:18px;margin:0 0 18px;border-bottom:1px solid var(--line);padding-bottom:12px}table{width:100%;border-collapse:collapse;font-size:13px}th,td{padding:11px;border-bottom:1px solid var(--line);text-align:left}th{background:#f8fafc;color:var(--muted);font-size:11px;text-transform:uppercase}.note{border-left:3px solid var(--teal);padding:12px 16px;background:#edf8f9;line-height:1.6}@media(max-width:800px){.kpis{grid-template-columns:repeat(2,1fr)}.content{padding:16px}section{overflow-x:auto}}</style></head><body><header class="cover"><p>${safe(data.organizationName)}</p><h1>Sales executive report</h1><p>Reporting period ${safe(date.toISOString().slice(0, 10))} · Stored conversation, workflow, and outcome data</p></header><main class="content"><div class="kpis"><div class="kpi"><span>Customers</span><strong>${safe(data.customerCount)}</strong></div><div class="kpi"><span>Consulting</span><strong>${safe(data.openCount)}</strong></div><div class="kpi"><span>Won</span><strong>${safe(data.wonCount)}</strong></div><div class="kpi"><span>Close rate</span><strong>${safe(Math.round(data.conversionRate * 100))}%</strong></div><div class="kpi"><span>Messages in period</span><strong>${safe(data.messageCount)}</strong></div></div><section><h2>Executive summary</h2><p class="note">${safe(data.executiveSummary)}</p></section><section><h2>Team performance</h2><table><thead><tr><th>Employees</th><th>Customers</th><th>Consulting</th><th>WON</th><th>Conversion</th></tr></thead><tbody>${employeeRows}</tbody></table></section><section><h2>Key insights</h2><table><thead><tr><th>Method</th><th>Conclusion</th><th>Sample size</th><th>Confidence</th></tr></thead><tbody>${insightRows}</tbody></table></section><section><h2>Transactions needing attention</h2><table><thead><tr><th>Customers</th><th>Sale</th><th>Segment</th><th>Lead score</th><th>Latest message</th></tr></thead><tbody>${attentionRows}</tbody></table></section><section><h2>Controls and methodology</h2><p>Quantitative data is calculated by application code from PostgreSQL. AI only explains insights and does not send messages, create calendar events, or close deals automatically.</p></section></main></body></html>`;
}
...
{\\Huge\\bfseries SALES EXECUTIVE REPORT}\\par\\vspace{0.7cm}
{\\Large Reporting period ${tex(date.toISOString().slice(0, 10))}}\\par\\vfill
{\\large Data from verified conversations, workflows, insights, and outcomes}\\par
...
Managed customers & ${data.customerCount} \\\\ Consulting & ${data.openCount} \\\\ Won & ${data.wonCount} \\\\ Close rate & ${Math.round(data.conversionRate * 100)}\\% \\\\ Messages in period & ${data.messageCount} \\\\ \\bottomrule\\end{tabular}
\\section*{Team performance}
\\rowcolors{2}{LightGray}{white}\\begin{longtable}{p{0.38\\linewidth}rrrr}\\toprule
Employees & Customers & Open & WON & Conversion \\\\ \\midrule\\endhead
${employeeRows}
\\bottomrule\\end{longtable}
\\section*{Key insights}
\\rowcolors{2}{LightGray}{white}\\begin{longtable}{p{0.18\\linewidth}p{0.52\\linewidth}rr}\\toprule
Method & Conclusion & Sample & Confidence \\\\ \\midrule\\endhead
${insightRows}
\\bottomrule\\end{longtable}
\\section*{Transactions needing attention}
\\rowcolors{2}{LightGray}{white}\\begin{longtable}{p{0.30\\linewidth}p{0.30\\linewidth}lr}\\toprule
Customers & Employees & Segment & Score \\\\ \\midrule\\endhead
${attentionRows}
\\bottomrule\\end{longtable}
\\section*{Methodology and controls}
Metrics are calculated by application code from PostgreSQL. AI only explains insights; the system does not allow AI to send messages, create calendar events, or close deals automatically.
\\end{document}`;
}
